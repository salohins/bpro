...
export default function AdaptiveTrendCloud() {
  ...
  const container = {
    ...
    show: {
      opacity: 1,
      transition: reduceMotion ? { duration: 0.01 } : { staggerChildren: 0.08, delayChildren: 0.06 },
    },
  };
  ...
  return (
    // ✅ no overflow-hidden — let effects bleed
    <section className="relative w-full py-20 md:py-24 bg-transparent text-white">
      {/* ✅ FX layer that bleeds beyond section */}
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">
        {/* make this taller than the section so the glow continues into next section */}
        <div className="absolute left-0 right-0 -top-40 -bottom-40">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_55%_18%,rgba(16,185,129,0.10),transparent_60%)]" />
          ...
          {/* optional extra soft fade so the bleed feels natural */}
          <div className="absolute inset-0 bg-gradient-to-b from-transparent via-black/[0.00] to-transparent" />
        </div>
      </div>
        ...
        >
          ...
          <motion.div variants={fadeLeft} className="lg:col-span-5 space-y-6">
            ...
            <p className="text-white/70 text-lg leading-relaxed max-w-xl">
              ...
              <span className="text-white/85 font-semibold">where you are</span> (bull / bear / neutral) and{" "}
              <span className="text-white/85 font-semibold">where breakouts are allowed</span> — before any signal prints.
            </p>
            ...
            <div className="pt-1 text-xs text-white/45 max-w-xl flex items-center gap-2">
              ...
              Next: filters decide if trading is permitted inside this structure.
            </div>
          </motion.div>
          ...
          <motion.div variants={fadeRight} className="lg:col-span-7">
            <div className="rounded-[30px] p-[1px] bg-gradient-to-b from-emerald-400/22 via-white/10 to-emerald-500/14 shadow-[0_0_70px_rgba(16,185,129,0.10)]">
              <div className="relative rounded-[30px] overflow-hidden bg-[#070707]/75 border border-white/10 backdrop-blur-2xl">
                <div className="relative z-10 px-6 py-4 border-b border-white/10 flex items-center justify-between">
                  <div className="text-sm font-semibold text-white/85">Adaptive Cloud in-action</div>
                ...
                <div className="relative h-[280px] sm:h-[360px] lg:h-[440px]">
                  ...
                  <div aria-hidden className="absolute inset-0 bg-[radial-gradient(circle_at_70%_18%,rgba(16,185,129,0.14),transparent_55%)]" />
            ...
            <div className="mt-4 text-xs text-white/45 max-w-3xl">
              The cloud is not “decoration” — it’s the regime map that every permission gate and signal is built on.
            </div>
          </motion.div>
        </motion.div>
      </div>
